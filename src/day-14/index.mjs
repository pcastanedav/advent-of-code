...
import {open, cp} from 'fs/promises'
...
async function applyRules (n) {
  const getPath = target => getInputPath(import.meta.url, `outputs/polymer-${target}`)
  ...
  await output.write(c[c.length - 1])
  await output.sync()
  await output.close()
  await cp(getPath('output'), getPath('input'))
}

async function transform(starting, n) {
  const startingFile = await open(getInputPath(import.meta.url, `outputs/polymer-input`), 'w')
